...
import com.puppycrawl.tools.checkstyle.pipeline.PipelineFileSetCheck;
...
public class CheckerTest extends AbstractModuleTestSupport {
    ...
    @Test
    public void testFileLengthCheckIsRoutedThroughPipelineFileSetCheck() throws Exception {
        final Checker checker = new Checker();
        final PackageObjectFactory factory = new PackageObjectFactory(
                new HashSet<>(), Thread.currentThread().getContextClassLoader());
        checker.setModuleFactory(factory);
        ...
        checker.setupChild(createModuleConfig(FileLengthCheck.class));

        final List<?> fileSetChecks = TestUtil.getInternalState(checker, "fileSetChecks", List.class);
        assertWithMessage("A single shared pipeline wrapper should be added")
                .that(fileSetChecks)
                .hasSize(1);
        assertWithMessage("FileLengthCheck should be routed to PipelineFileSetCheck")
                .that(fileSetChecks.getFirst())
                .isInstanceOf(PipelineFileSetCheck.class);
    }

    @Test
    public void testLineLengthAndFileLengthChecksShareSinglePipelineWrapper() throws Exception {
        final Checker checker = new Checker();
        final PackageObjectFactory factory = new PackageObjectFactory(
                new HashSet<>(), Thread.currentThread().getContextClassLoader());
        checker.setModuleFactory(factory);
        ...
        checker.setupChild(createModuleConfig(FileLengthCheck.class));
        checker.setupChild(createModuleConfig(LineLengthCheck.class));

        final List<?> fileSetChecks = TestUtil.getInternalState(checker, "fileSetChecks", List.class);
        assertWithMessage("Size checks should share a single pipeline wrapper")
                .that(fileSetChecks)
                .hasSize(1);
        assertWithMessage("Wrapper type should be PipelineFileSetCheck")
                .that(fileSetChecks.getFirst())
                .isInstanceOf(PipelineFileSetCheck.class);
    }
