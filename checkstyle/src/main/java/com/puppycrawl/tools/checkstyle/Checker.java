...
public class Checker extends AbstractAutomaticBean implements MessageDispatcher, RootModule {
    ...
    /** Shared pipeline wrapper for file-level size checks. */
    private PipelineFileSetCheck pipelineFileSetCheck;
    ...
    protected void setupChild(Configuration childConf)
        ...
        if (child instanceof FileSetCheck fsc) {
            fsc.init();
            if (fsc instanceof FileLengthCheck || fsc instanceof LineLengthCheck) {
                addFileLevelSizeCheckToPipeline((AbstractFileSetCheck) fsc);
            }
            else {
                addFileSetCheck(fsc);
            }
    ...
    /**
     * Adds configured file-level size checks to a shared pipeline wrapper so
     * existing configuration continues to work without XML changes.
     *
     * @param check configured file-level size check
     */
    private void addFileLevelSizeCheckToPipeline(AbstractFileSetCheck check) {
        if (pipelineFileSetCheck == null) {
            pipelineFileSetCheck = new PipelineFileSetCheck(false);
            addFileSetCheck(pipelineFileSetCheck);
        }
        pipelineFileSetCheck.addConfiguredSizeCheck(check);
    }
