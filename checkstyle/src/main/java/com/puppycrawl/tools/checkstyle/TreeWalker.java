...
import com.puppycrawl.tools.checkstyle.pipeline.CheckPipeline;
...
public final class TreeWalker extends AbstractFileSetCheck implements ExternalResourceHolder {
    ...
    /**
     * Pipeline for metrics and sizes checks that must bypass direct TreeWalker dispatch.
     * Checks intercepted here are never added to {@link #ordinaryChecks} or the token maps.
     */
    ...
    public void setupChild(Configuration childConf)
        ...
        if (module instanceof AbstractCheck check) {
            check.init();
            if (CheckPipeline.isPipelineManaged(check)) {
                pipeline.addManagedCheck(check);
            }
            else {
                registerCheck(check);
            }
    ...
    protected void processFiltered(File file, FileText fileText) throws CheckstyleException {
        ...
        if (!ordinaryChecks.isEmpty() || !commentChecks.isEmpty() || !pipeline.isEmpty()) {
            ...
            if (!skip) {
                ...
                if (!pipeline.isEmpty()) {
                    violations.addAll(pipeline.processAst(rootAST, contents));
                }
                if (filters.isEmpty()) {
                    addViolations(violations);
                }
